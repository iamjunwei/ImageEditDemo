...
public class DemoImageViewStable1 extends View {

    private static final int SCALE_POINT_RADIUS = 80;
    ...
    public DemoImageViewStable1(Context context) {
        super(context);
        init();
    }

    public DemoImageViewStable1(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public DemoImageViewStable1(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void handleMoveEventTouchPic(MotionEvent event) {
        bitmapLeft += (event.getX() - dx);
        bitmapTop += (event.getY() - dy);
